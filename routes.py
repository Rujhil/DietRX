from core import app, fgname2id
...
def index():
    ...
    foods = Food.query.limit(100).all()
    food_of_the_day = foods[date.today().day % len(foods)] if foods else None
...
def search():
	...
	page_data = Pagination(
		page, NUM_PER_PAGE, [0]*results['hits']['total']['value'], request, 'search')
...
def chemical_search():
    results, search_by_smiles = process_search_query(request.args)
...
CSV_MAP = {
    ...
    'meategg':     'top50_MeatEgg.csv',
    'plant':       'top50_Plant.csv',
...
def top50_csv(slug):
    ...
    fname = CSV_MAP.get(slug.lower())
    if not fname:
        return jsonify([])

    # ✅ Update this path to match your structure
    csv_path = os.path.join(app.root_path, 'analytics', fname)
